...
class Conv2dLayer(torch.nn.Module):
    def __init__(self,
        ...
        self.register_buffer('resample_filter', upfirdn2d.setup_filter(resample_filter, device=torch.device('cuda:0')))
...
class SynthesisLayer(torch.nn.Module):
    ...
    def forward(self, x, w, noise_mode='random', fused_modconv=True, gain=1):
        ...
        if self.use_noise and noise_mode == 'random':
            ...
            noise = torch.randn([x.shape[0], 1, self.resolution[0], self.resolution[1]], device=x.device) * self.noise_strength
